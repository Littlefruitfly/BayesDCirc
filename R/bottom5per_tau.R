...
##' mu_E = rnorm(1000)
##' mu_F = rnorm(1000)
##' bottom5per_tau(mu_E, mu_F)

bottom5per_tau <- function(mu_E, mu_F) {
    ...
    a_tau2_0 <- nrow(sorted_df)
    b_tau2_0 <- (1/2) * (sum(sorted_df$mu_E^2) + sum(sorted_df$mu_F^2))
    prior <- data.frame(a_tau2_0, b_tau2_0)
    return(prior)
}
